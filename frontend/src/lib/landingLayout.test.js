...
describe('landing page layout guardrails', () => {
  it('keeps the decorative orb out of the hero grid flow', () => {
    expect(appSource).toContain('className="landing-orb"')
    expect(styles).toMatch(/\.landing-orb\s*\{[^}]*position:\s*absolute;/s)
    expect(styles).toMatch(/\.landing-orb\s*\{[^}]*pointer-events:\s*none;/s)
  })

  it('keeps the unauthenticated hero constrained and two-column on desktop', () => {
    expect(styles).toMatch(/\.landing-hero\s*\{[^}]*width:\s*min\(100%,\s*1240px\);/s)
    expect(styles).toMatch(/\.landing-hero\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s*minmax\(280px,\s*410px\);/s)

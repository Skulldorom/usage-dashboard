...
function UsageCard({ item }) {
  ...
  return <Card variant="outlined" sx={{ height: '100%' }}><CardContent><Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2}><Box><Typography variant="overline" color="text.secondary">{config.provider}</Typography><Typography variant="h6">{config.label}</Typography></Box><Chip color={color} label={latest?.status || 'not polled'} size="small" /></Stack><Typography sx={{ mt: 2, mb: 2 }} color="text.secondary">{latest?.summary || 'No usage snapshot yet. Hit refresh; summon data from the void.'}</Typography><Stack spacing={2}>{(latest?.metrics || []).map((metric) => { const percent = metricPercent(metric); return <Box key={metric.label}><Stack direction="row" justifyContent="space-between"><Typography variant="body2">{formatMetricLabel(metric.label)}</Typography><Typography variant="body2" color="text.secondary">{String(metric.value ?? '—')} {metric.unit || ''}</Typography></Stack>{percent !== null && <LinearProgress variant="determinate" value={percent} sx={{ mt: 0.75 }} />}</Box> })}</Stack>{latest?.error && <Alert severity="error" sx={{ mt: 2 }}>{latest.error}</Alert>}<UsageHistory config={config} latest={latest} /></CardContent></Card>
}
